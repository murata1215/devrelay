...
- **端末インタフェースモード**: プロジェクトごとの「端末」トグルで Agent SDK の代わりに PTY 経由（node-pty + @xterm/headless）で `claude --continue` を起動。ターミナルからの対話セッションを自然継続、都度起動・都度終了で常駐ゼロ。`Project.terminalMode` カラム、`GET/PUT /api/projects/:id/terminal-mode`、完了検出は「最終出力から 10 分無音」がメイン基準。Phase 1 は Linux Agent のみ、tool 承認カード連動は Phase 3
