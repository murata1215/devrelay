...
- **Devin CLI 対応**: Devin for Terminal を spawn パターンで統合（`--prompt-file` + `-r <session-id>` 明示的セッション継続）、CLAUDE.md 互換、`x` コマンドでセッションクリア
- **AI ツール自動検出**: Agent 起動時に `which`/`where` でインストール済み CLI を自動検出し config.yaml に追加（手動設定不要）
- **端末インタフェースモード**: プロジェクトごとの「端末」トグルで Agent SDK の代わりに PTY 経由（`@homebridge/node-pty-prebuilt-multiarch` + `@xterm/headless`）で `claude` を起動。都度起動・都度終了で常駐ゼロ。Linux + Windows（CLI 版）で動作。SDK のセッションを `claude --resume <id>` で復元するので、SDK と端末モードが `~/.claude/projects/<hash>/sessions/<id>.jsonl` を共有して双方向に文脈継続。250ms ポーリングで信頼プロンプトや入力プロンプトを無音中も検出、番号付き選択肢（tool 承認・AskUserQuestion・trust dialog）は既存 WS 承認カード経路にブリッジ（WebUI/Discord/Telegram は変更なし）。完了検出は「画面アイドル 1.5 秒 + プロンプト復帰」。Windows インストーラーは `prebuild-install` が `conpty.node` をスキップした場合、ABI 一致の tarball を GitHub Releases から自動 download する fallback 付き
