...
- 実行: `devin -p --permission-mode dangerous`（非対話 + 全ツール自動承認）
- プロンプト: stdin 経由（プロセスリストに非表示）
- PATH: コマンドのディレクトリを自動追加（サービス実行時の PATH 不足を回避）
- 有効化: Agent 起動時に自動検出（`detectAndUpdateAiTools()`）、または `~/.devrelay/config.yaml` の `aiTools.devin.command` を手動設定
