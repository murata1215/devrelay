...
- **node-pty の Windows プレビルド**: monorepo root の `package.json` の `pnpm.onlyBuiltDependencies` に `node-pty` を追加（native binary build が必要）。Windows Agent は Phase 2 で展開時にプレビルド対応を要確認
