...
- **セッション継続は `--resume <SDK session id>`** で行う（`--continue` は使わない）。`.devrelay/claude-session-id` に SDK が保存した UUID を `loadClaudeSessionId(projectPath)` で読み、CLI の `--resume` に渡す
  - 両者は `~/.claude/projects/<hash>/sessions/<id>.jsonl` を共有しているため互換。CLI が同じファイルに追記 → 次回 SDK 起動時もそのまま読み取れる = 双方向の継続が成立
  - `--continue` は cwd ベースで「No conversation found to continue」即死するため不採用
  - session id が無い場合（fresh プロジェクト）は `--resume` を付けず、CLI が新規セッションを採番（次回以降は SDK がその id を `.devrelay/claude-session-id` に保存して継続できるようになる）
...
- **完了検出は「画面アイドル + プロンプト復帰 + 新規バレットあり」**: 500ms setInterval で「最後の画面変化から 1.5 秒以上経過 + `detectPromptReady` true + 承認待ち無し + `newBullets > 0`」を判定。onData 駆動で完了判定すると入力ボックスの `❯` が常駐するため永遠に発火しない
  ...
  - **3 段階 safety net**: (1) `IDLE_FOR_COMPLETION_MS = 1500ms` 画面アイドル、(2) `FIRST_BULLET_TIMEOUT_MS = 10 分` バレット未到来時の強制完了（画像複数や 100k+ token 履歴では 5 分超は普通なので 10 分）、(3) `IDLE_TIMEOUT_MS = 10 分` の onData ベース最終 timeout
...
- **応答配信は「バレット逐次ストリーミング + 思考ハートビート + 最終整形ブロック」**: 完了 check 内で baseline 超え + 未送信の `●` バレット行を発見次第 `opts.onOutput` で WebUI に配信（200 char で truncate）。バレット未到来 + 直近 20s バレット送信なし + 30s 間隔 max で `extractThinkingIndicator(rendered)` を抽出して `⏳ [Xs 経過] Doing (Xs · Y tokens)` 形式のハートビート配信。完了時は `extractClaudeResponse(rendered, baselineBulletMap)` で「baseline の count を超える最初のバレット」から全本文を抜き出して送信。バレットヘッダ部分はストリーミング分と最終分で 1 回ずつ重複するが本文の supporting text は最終応答にしか含まれない（#228 続編 2）
...
- **任意の選択肢プロンプトは全て WS カード経由でユーザーに委ねる**（#228 続編 2）: 設計思想「ターミナルモードは Claude CLI の薄い UI ラッパ」に基づき、agent が自動判断せずユーザーに選択させる
  ...
  - **起動時のシステム選択肢** (`detectStartupChoicePrompt`): `Enter to confirm · Esc to cancel` + 番号付き選択肢の共通パターン（trust folder / resume from summary / 将来追加されるもの）。カーソル `❯` が option 1 行頭に乗る形式に対応するため `extractChoicePrompt` の regex 先頭に `[❯>]?` 許容を追加
  - 両者とも `extractChoicePrompt` で `{question, options}` に分解 → `onToolApprovalRequest` callback（SDK と共通の `ToolApprovalRequest` 形式）で server → WS 承認カード → ユーザー応答が `resolveToolApproval` 経由で `terminalApprovalResolvers` map に届き、`<choice>\r` を PTY に書き戻す。Server/WebUI/Discord/Telegram は変更ゼロ
...
- 実行: `devin [-r <session-id>] -p --permission-mode auto|dangerous --prompt-file <tmp>`
...
- パーミッション: plan → `auto`、exec → `dangerous`。`-r` による resume でも `--permission-mode` の変更が正常に効く
- 会話履歴: 非 Claude ツールでは常にプロンプトに会話履歴を含める（`isClaudeSdk` 判定で Claude は従来通り SDK --resume）
