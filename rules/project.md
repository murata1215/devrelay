...
`Project.terminalMode` が true の場合、Agent は Agent SDK の `query()`（`-p` 相当）ではなく PTY 経由で `claude --continue` をインタラクティブ起動する（`agents/<os>/src/services/terminal-runner.ts`）。1 要求 1 セッションで都度起動・都度終了。
...
- **完了検出**: 「最終出力から 10 分無音」をメイン基準、プロンプト復帰検出は補助
  - `onData` 発火ごとに `resetIdleTimer()` を呼ぶ → 長文出力中・思考中・tool 連発中はタイムアウトしない
  - プロンプト復帰検出は ANSI 除去後の末尾走査で `> ` / `│ > ` を判定（誤検出を避けるため起動後 2 秒のクールダウンあり）
...
- **tool 承認の扱い（Phase 1）**: 自動承認 ON 時は `--dangerously-skip-permissions` を付与して CLI 側で承認プロンプトを抑止。OFF 時に CLI が承認プロンプトを出した場合は `2\r`（いいえ）を自動送信（Phase 3 で WS 承認カード連動に拡張予定）
- **Ask 無効化**: PTY モードでは SDK `disallowedTools` が使えないため、画面パースで AskUserQuestion プロンプトを検出 → Ctrl+C 中断 + エラー返却
- **既存挙動の保証**: `terminalMode = false`（デフォルト）の場合は完全 no-op。新規 import・型追加以外、既存実行パスには一切影響しない
...
- **Windows での PTY プリビルド手動 fallback**: Windows では `pnpm rebuild @homebridge/node-pty-prebuilt-multiarch` が走っても `prebuild-install` が `build/Release/` に `conpty.node` を配置しないケースがある（Node 24 ABI 137 + pnpm 10 で確認、原因は homebridge フォーク同梱の Linux prebuilds を check-prebuild.js が誤検出してスキップする可能性）。`install-agent.ps1` と `agents/linux/src/services/connection.ts` の Windows update buildSteps に **`build/Release/conpty.node` の存在確認 → 無ければ GitHub Releases から ABI 別 tarball を直接 download して `tar -xzf` 展開** するフォールバックを組み込む。`tar.exe` は Windows 10+ 標準同梱なので追加ツール不要
