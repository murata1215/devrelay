...
- **完了検出は「画面アイドル + プロンプト復帰 + 新規バレットあり」**: 500ms setInterval で「最後の画面変化から 1.5 秒以上経過 + `detectPromptReady` true + 承認待ち無し + `newBullets > 0`」を判定。onData 駆動で完了判定すると入力ボックスの `❯` が常駐するため永遠に発火しない
  - **`newBullets > 0` のガードが必須**: Claude の思考フェーズ初期は PTY が無音になる（API リクエスト送信 → first token までの latency）。この期間を「処理完了」と誤判定して `/exit` を queue してしまう症状が Windows mviewer で確定（#228 続編）。応答 1 個目の `●` バレットが出るまでは完了とみなさない。完全無応答ケースは既存の `IDLE_TIMEOUT_MS = 10 分` が safety net
...
- **応答抽出は `●` バレットブロックベース**: 実行中はストリーム送信せず、完了時に `extractClaudeResponse(rendered, baselineBulletCount)` で「baseline 時点以降に出現した `●` メッセージ」を抜き出して送信。途中ストリーミングは入力ボックスの常駐 UI が画面中央で書き換わるため delta が破綻するので諦めた
...
- **tool 承認の WS カード連動**: 番号付き選択肢プロンプトを `extractChoicePrompt` で `{ question, options }` に分解 → 既存の `onToolApprovalRequest` callback（SDK と共通の `ToolApprovalRequest` 形式）で server へ → server は既存承認カード経路で WebUI/Discord/Telegram へ → ユーザー応答が `resolveToolApproval` 経由で `terminalApprovalResolvers` map に届き、`<choice>\r` を PTY に書き戻す。Server/WebUI/Discord/Telegram は変更ゼロ
- **信頼プロンプト自動承認**: `--dangerously-skip-permissions` を付けても初回ワークスペースでは出るため、`detectTrustPrompt` で検出して Enter を自動送信。"ユーザーが端末モードを ON にした" が明示的承認のトリガなので無人自動承認には該当しない
