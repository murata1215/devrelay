...
- セッション継続: `-c` は **plan モードのみ** 付与。exec モードでは新規セッション（`-c` で resume すると前セッションの permission-mode が保持されてしまうため）
- パーミッション: plan モード → `auto`（読み取り専用）+ `-c`、exec モード → `dangerous`（全承認）+ 新規セッション。`plan` は Devin 未対応
