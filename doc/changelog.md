...
**追加修正**: nohup 更新スクリプトの bash プロセスが `&` 後も終了しない問題を修正。`nohup node ... &` の後に `disown` を追加し、バックグラウンドジョブの参照を解放して bash が即終了するようにした（Linux + macOS）。

> **検証メモ**: disown 修正の動作確認は `u` コマンド2回実行で検証。1回目で新コードを取得、2回目で disown 付きスクリプトが実行され bash プロセスが即終了することを確認する。
