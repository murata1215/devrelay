...
- `doc/devlog/` ディレクトリが存在しなかったため新設し、直前の #324 サイクルの記録を `doc/devlog/2026-08-23_062023.md`（要求/実行/検証/発見の4段落）として作成
- `doc/devlog/INDEX.md`（`# Devlog INDEX` 見出し + 索引1行）を新規作成
- コード変更・ビルド・commit/push は今回のファイル作成自体には伴わない（ドキュメントのみ）
...
- **原因（コード確認で確定）**: `w` はコマンドとしては正しく `{ type: 'exec' }` に変換・送信されている（＝コマンド解釈は無罪）。`w` の実体は `command-parser.ts` に固定展開される**ワンショット exec プロンプト**（「今回の変更を changelog/README 等へ追記して**コミット＆プッシュ**」）で、**実装後の後処理専用**。しかし送信時の作業ツリーがクリーン（`git diff` 空・HEAD が push 済み・未追跡は画像のみ）だと、**コミットすべき変更が 1 つも無い** → agent が「“今回の変更”が何を指すのか不明」と受け取り、存在しないプランを探して「プランをください」ループに陥っていた
- **修正**: `W_COMMAND_PROMPT` 定数を新設し、冒頭にガード文を追加。「まず `git status`/`git diff` で未コミットの変更を確認し、**変更が無ければ存在しないプランを推測せず『コミット対象の変更はありません』とだけ報告して終了**（追加の実装・調査は不要）。変更がある場合のみドキュメント更新＋コミット/プッシュ」に変更。あわせて重複していた 2 箇所（`parseCommand` の Step 0.6 と `parseShortcut` のフォールバック）を共有定数に集約し、文言のドリフトを防止
- **対象**: `apps/server/src/services/command-parser.ts`（1 ファイル）+ changelog.md。Agent / shared / DB / WebUI 変更なし。`pnpm --filter @devrelay/server build` 成功、dist にガード文（`コミット対象の変更はありません`）×1・`W_COMMAND_PROMPT` ×3 を確認
- **反映**: `.ts` 変更のため **`pm2 restart devrelay-server` 要**（Agent 再起動不要）
- **教訓**: `w` は「実装」トリガーではなく後処理専用（実装は `e`/`exec`）。変更ゼロで `w` を送ると空振りする仕様だった。ワンショット exec プロンプトには「対象が無ければ推測せず終了」のガードを入れないと、フレッシュな作業ツリーで無限に指示を求めるループになる
...
- **原因（不具合ではなく仕様の隙間）**: DB タイムスタンプ確認で該当 exec は **59 秒で完了**（直前のタスクも 57 秒 / 20 秒）。#276 のハートビートは `setInterval(..., 60_000)` で**初回発火が 1 分後**のため、**1 分未満で終わるタスクでは 1 回も発火しない**。コード自体は正しく動作していた
- **修正**: ハートビート間隔を **60 秒 → 30 秒** に短縮し、1 分未満は秒表示に（`⏳ Devin 実行中... (30秒経過 / 上限15分)` → 90 秒で `(1分経過 / 上限15分)`）。典型的な 30 秒〜1 分のタスクでも最低 1 回は進捗が出る
- **対象**: `agents/{linux,macos,windows}/src/services/ai-runner.ts`（3 ファイル、ハートビートブロックのみ）。#276/#277 の他機構（`⏳` prefix 除外・上限タイマー・ATIF ポーラー・後始末）は無変更。サーバー・shared・connection.ts・DB 変更なし
- **反映**: **Agent のみ、サーバー再起動不要**。Devin 利用マシン（TISA 02 / 03 TISA）で `u`→`u`
- **補足**: ATIF ステップ要約（3 秒ポーリング）が出ないのは、対象 devin が `--export` 非対応の場合（agent.log の `🔎 --export support: false` で判別可）で正常
...
- **修正 4**: Windows restart ハンドラの VBS 起動を遅延方式に変更。`start-agent.cmd` が `>> agent.log` でファイルを排他ロック保持するため、旧プロセスと新プロセスが同時に起動すると新 CMD がログファイルをオープンできずサイレントに失敗していた。`restart-agent.vbs` を動的生成し `WScript.Sleep 2000` で 2 秒待機してから `start-agent.cmd` を起動（`cmd.exe /c "ping ... & wscript"` 方式は spawn の引数クォーティングで壊れるため VBS に Sleep を組み込む方式を採用）。旧プロセスは即座に exit（100ms）
- 対象: `apps/server/src/services/agent-manager.ts`, `agents/linux/src/services/connection.ts`
- **設計判断**: `connectedAgents` への早期登録は、トークン検証さえ通れば正当な接続。以降の DB 操作が失敗しても ws は有効なので、disconnect handler のレースセーフティが最優先。Windows の遅延リスタートは Update ハンドラ（PowerShell が旧プロセスを kill してから起動）と異なり、restart は軽量に即 exit → VBS Sleep → 起動で対処
...
- **セッション管理**: `-c` → `-r <session-id>` に改善。`devin list --format json` で実行後にセッション ID を取得・保存。`x` コマンドでクリア
- **非 Claude の会話履歴注入**: Devin/Gemini 等では常にプロンプトに会話履歴を含める（Claude は SDK --resume で不要）
- 全 Agent（Linux/macOS/Windows）の `ai-runner.ts` + `connection.ts` + `session-store.ts` に実装
- **設計判断**: `-r` は明示的セッション ID 指定のため permission-mode 変更が確実に効く（`-c` の「前セッション設定保持」問題を解消）。一時ファイルは close ハンドラで確実に削除
...
- **トグルスイッチ化**: ⚡ アイコンのみ → 「自動承認」ラベル + スライドスイッチ
  - ON: amber 色テキスト + amber スイッチ（ノブ右寄せ）
  - OFF: faint 色テキスト + グレースイッチ（ノブ左寄せ）
- **MachinesPage と同スタイル**: Agent Settings 内のスイッチと統一的な UI

#### 変更ファイル
- `apps/web/src/pages/ChatPage.tsx` — ヘッダートグル UI 変更
...
- **セッションスコープ**: ルールは exec セッション単位で有効（次の exec で自動リセット）。永久保存ではなく安全
- Linux / macOS Agent 両方で対応

#### ツール承認カード配色修正（ダークモード対応）
- カード枠: `border-2` → `border`（細く）、`border-yellow-500/50` → `border-amber-500/30`（控えめに）
- カード背景: ライト/ダーク別指定（`bg-amber-900/10 dark:bg-amber-500/5`）
- ファイルパス部分: ダークモードのコントラスト改善（`dark:bg-white/10`）
- ボタン: `flex-wrap` でモバイル折り返し対応、「以降すべて許可」を落ち着いたスレート色に

#### Approvals タブ履歴の展開表示
- 承認履歴エントリをクリックで全文表示（折りたたみ/展開トグル）
- Bash コマンドの 60 文字制限を削除（CSS truncate に任せ、展開時に全文表示）
...
#### 概要
ダークモード切替ボタンの横に通知音のオン/オフトグルボタンを追加。Settings ページに行かなくてもヘッダーから素早く切り替え可能に。

#### 変更内容
- デスクトップ・モバイル両方のヘッダーにスピーカー/ミュートアイコンボタンを追加
- 既存の `notification-sound.ts` ユーティリティを再利用（localStorage `devrelay-notification-sound`）

#### 変更ファイル

| ファイル | 変更内容 |
|---------|---------|
| `apps/web/src/components/Layout.tsx` | 通知音トグルボタン追加（デスクトップ版 + モバイル版） |

---

### #157: コマンド結果の複数ブラウザ間同期 (2026-03-14)

#### 概要
#155/#156 でユーザーのコマンド入力は他ブラウザに同期されるようになったが、コマンドの実行結果（`h` のヘルプ表示、`w` の作業状況など）は送信元ブラウザにしか返されていなかった問題を修正。

#### 変更内容
- `web.ts` の `executeCommand()` 結果を送信元のみに返す処理を、同じセッションの全 Web クライアントにブロードキャストするように変更
- セッション未参加時（参加前のコマンド等）は従来通り送信元のみに返す

#### 変更ファイル

| ファイル | 変更内容 |
|---------|---------|
| `apps/server/src/platforms/web.ts` | コマンド結果の `web:response` を全セッション参加者にブロードキャスト |

---

### #156: 全コマンドの複数ブラウザ間同期 (2026-03-14)

#### 概要
#155 で導入した複数ブラウザ間チャット同期が `ai:prompt` タイプのみに限定されていたため、`x`（exec）や `e`、`w` 等のコマンドが他ブラウザに表示されなかった問題を修正。

#### 変更内容
- `web.ts` のブロードキャスト条件 `if (command.type === 'ai:prompt')` を削除し、全コマンドタイプを他ブラウザにブロードキャストするように変更

#### 変更ファイル

| ファイル | 変更内容 |
|---------|---------|
| `apps/server/src/platforms/web.ts` | ブロードキャスト条件を `ai:prompt` 限定から全コマンド対象に変更 |
