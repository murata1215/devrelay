...
- **Windows PowerShell 更新スクリプトをファイル実行方式に変更**: `spawn('powershell', ['-Command', script])` → `update.ps1` ファイルに書き出して `spawn('powershell', ['-ExecutionPolicy', 'Bypass', '-File', scriptPath])` で実行。`-Command` では stop コマンド内の二重引用符（`Get-CimInstance -Filter "Name='node.exe'"`）がコマンドライン引数パースと競合し、PowerShell が構文エラーで終了していた（`update.log` が一切生成されなかった）
