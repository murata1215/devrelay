#!/usr/bin/env node

import { Command } from 'commander';
import { setupCommand } from './commands/setup.js';
import { statusCommand } from './commands/status.js';
import { projectsCommand } from './commands/projects.js';
import { startCommand } from './commands/start.js';
import { logsCommand } from './commands/logs.js';
import { uninstallCommand } from './commands/uninstall.js';
import { getConfigDir } from '../services/config.js';

const program = new Command();

program
  .name('devrelay')
  .description('DevRelay Agent for Windows - Remote AI CLI development hub')
  .version('0.1.0');

program
  .command('setup')
  .description('Initial setup and configuration')
  .action(setupCommand);

program
  .command('status')
  .description('Show agent status')
  .action(statusCommand);

program
  .command('start')
  .description('Start the agent')
  .action(startCommand);

program
  .command('stop')
  .description('Stop the agent')
  .action(() => {
    console.log('Stopping agent...');
    console.log('Use Task Manager to stop the agent process, or:');
    console.log('  taskkill /IM node.exe /F (Warning: kills all Node processes)');
  });

program
  .command('restart')
  .description('Restart the agent')
  .action(() => {
    console.log('Restarting agent...');
    console.log('Stop and start the agent manually:');
    console.log('  devrelay stop');
    console.log('  devrelay start');
  });

program
  .command('logs')
  .description('View agent logs')
  .option('-f, --follow', 'Follow log output')
  .option('-n, --lines <number>', 'Number of lines to show', '50')
  .action(logsCommand);

program
  .command('projects')
  .description('Manage projects')
  .argument('[action]', 'Action: list, add, remove, scan')
  .argument('[path]', 'Project path (for add/remove)')
  .option('--ai <tool>', 'Default AI tool', 'claude')
  .action(projectsCommand);

program
  .command('config')
  .description('Open config file in editor')
  .action(async () => {
    const { execSync } = await import('child_process');
    const configPath = `${getConfigDir()}\\config.yaml`;
    // Use notepad as default editor on Windows
    try {
      execSync(`notepad "${configPath}"`, { stdio: 'inherit' });
    } catch {
      console.log(`Config file: ${configPath}`);
      console.log('Open this file in your preferred editor.');
    }
  });

program
  .command('token')
  .description('Show or regenerate connection token')
  .action(async () => {
    const { loadConfig } = await import('../services/config.js');
    const config = await loadConfig();
    console.log(`Current token: ${config.token || '(not set)'}`);
  });

program
  .command('uninstall')
  .description('Uninstall DevRelay Agent')
  .action(uninstallCommand);

program.parse();
