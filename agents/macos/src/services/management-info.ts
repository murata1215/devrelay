...
function generateDarwinInfo(): ManagementInfo {
  ...
  const commands: ManagementCommand[] = [
    { type: 'logs', label: 'ログ', command: `tail -f ${logFile}` },
    { type: 'stop', label: '停止', command: 'pgrep -f "\\.devrelay.*index\\.js" | grep -v "^$$\\$" | xargs kill' },
    {
      type: 'restart',
      label: '再起動',
      ...
      command: `NODE_BIN="${nodePath}"; [ ! -x "$NODE_BIN" ] && NODE_BIN=node; pgrep -f "\\.devrelay.*index\\.js" | grep -v "^$$\\$" | xargs kill 2>/dev/null || true; sleep 1; cd ${path.dirname(agentIndex)} && nohup "$NODE_BIN" ${agentIndex} < /dev/null >> ${logFile} 2>&1 &`,
    },
