...
interface DevinTurnDigest {
  ...
  /**
   * #368 Phase1-3: `chisel/tool_failure: {"reason":"Blocked"}` と対になった拒否コマンド一覧
   * （`devin-atif.ts` の `extractBlockedCommands()` をそのまま透過、空配列は「見つからなかった」の意）。
   */
 ...
 * fs I/O のみ担当し、パース本体は devin-atif.ts の buildAtifDigest() に委譲する。
...
function readDevinTurnDigest(exportPath: string, lang: Language = DEFAULT_CHAT_LANGUAGE, offset: number = 0): DevinTurnDigest | null {
  ...
  return {
    ...
    blockedCommands: digest.blockedCommands,
  };
}
...
async function sendPromptToAiSdk(
  ...
  const sdkOptions: Parameters<typeof query>[0]['options'] = {
    ...
    model: options.model,
    // #355: `return` だけでは claude サブプロセスが死なないため（sdk.mjs 実測: signal は
    // spawn() に渡され、abort リスナーが SIGTERM を送る）、外部から確実に中断できるように
    // AbortController を渡す。cancelAiSession() の `c` コマンド経路と sdk-loop-guard の
    // 強制打ち切りの両方がこれを使う。
  ...
  // セッション resume（core#336: 優先順位判定は resume-priority.ts に集約）。
  // 注: explicit resumeSessionId + forceNewSession が同時に来た場合は connection.ts 側の
  // バグの可能性が高いとみなし二重防御として resume させない（decideResume() 自体は explicit を
  // forceNewSession より優先する設計のため、ここでは decideResume() を呼ぶ前に明示的に弾く）。
  ...
  try {
    for await (const message of sdkQuery) {
      ...
      if (m.session_id && !result.extractedSessionId) {
        ...
        if (options.persistProjectState !== false) {
          saveClaudeSessionId(projectPath, m.session_id, options.agentScopeId).catch(err => {
...
export async function sendPromptToAi(
  ...
  // #368 Phase1-3: ATIF の各エントリに chisel/tool_failure 等の「拒否」マーカーが付与されている場合に
  // 実際に拒否されたコマンド文字列を抜き出したもの（extractBlockedCommands()）。同上の理由で
  // 関数スコープの let。空配列＝拒否コマンドを特定できなかった（rejectionEvidence 等にフォールバック）。
  let devinAtifBlockedCommands: string[] = [];
  ...
  if (aiTool === 'gemini') {
    ...
    let devinDegradedReason: 'execPermission' | null = null;
    ...
    // #368 Phase2a: Devin は常に exec 相当（dangerous）で起動する。
    const devinTurnPermissionMode: string | null = devinHasPermissionMode ? 'dangerous' : null;

    // 保存済み Devin セッション ID があれば -r で resume（前回と同じ権限モードのときのみ、
    // モードが変わっていたら resume せず新規セッションで開始する）
    ...
    if (devinDegradedReason === 'execPermission') {
      ...
      // #346: --permission-mode 非対応時は「このマシンで何が使えるか」をプロセス寿命中 1 回だけ追送する
  ...
  const runKillPlan = (stage: KillStage) => {
    ...
    console.log(`[${aiTool}] kill stage=${stage}: ${plan.note}`);
    if (plan.command) {
      const killer = spawn(plan.command.file, plan.command.args, { stdio: 'ignore', windowsHide: true });
      killer.on('error', (err) => {
        console.warn(`[${aiTool}] taskkill failed to start (${err.message}), falling back to proc.kill(${plan.fallbackSignal})`);
        try { proc.kill(plan.fallbackSignal); } catch {}
      });
      killer.on('close', (code) => {
        console.log(`[${aiTool}] taskkill exited with code ${code}`);
      });
    } else {
      try { proc.kill(plan.fallbackSignal); } catch {}
    }
  ...
  const requestKill = (reason: string) => {
    ...
    killRequested = true;
    killStage = 'term';
    console.log(`[${aiTool}] kill requested (reason=${reason})`);
    runKillPlan('term');
    ...
    killGiveUpTimer = setTimeout(() => {
      if (turnEnded) return;
      console.warn(`[${aiTool}] ⚠️ process still alive after kill escalation, forcing turn finalize (pid=${proc.pid ?? '?'})`);
      forcedFinalize = true;
  ...
  if (session) {
    ...
    if (prevProcess && prevProcess !== proc && prevProcess.exitCode === null && prevProcess.pid) {
      console.warn(`[${aiTool}] ⚠️ previous process still alive (pid=${prevProcess.pid}), killing its tree before overwriting`);
  ...
  if (options.resumeSessionId) {
    startupTimeoutTimer = setTimeout(() => {
      if (fullOutput.length === 0 && !completionSent) {
        ...
        result.resumeFailed = true;
        requestKill('resumeStartupTimeout');
      }
  ...
  return new Promise<AiRunResult>((resolve) => {
      ...
      for (const line of lines) {
        ...
        try {
          ...
          if (parsed.sessionId) {
            ...
            // #348: persistProjectState===false（一時セッション）なら projectPath 上には書かない
            if (options.persistProjectState !== false) {
              saveClaudeSessionId(projectPath, parsed.sessionId, options.agentScopeId).catch(err => {
                console.error(`Failed to save session ID:`, err);
              });
            }
          }
    ...
    proc.on('exit', (code, signal) => {
      processExited = true;
      console.log(`[${aiTool}] 'exit' received (code=${code}, signal=${signal}), waiting for stdio close`);
      if (turnEnded) return;
      exitFlushTimer = setTimeout(() => {
        if (turnEnded) return;
        console.warn(`[${aiTool}] ⚠️ 'close' not fired ${killTimings.exitFlushGraceMs}ms after exit, destroying stdio`);
        try { proc.stdin?.destroy(); } catch {}
        try { proc.stdout?.destroy(); } catch {}
        try { proc.stderr?.destroy(); } catch {}
        syntheticCloseTimer = setTimeout(() => {
          if (turnEnded) return;
          console.warn(`[${aiTool}] ⚠️ 'close' still not fired after stdio destroy, synthesizing close`);
    ...
    proc.on('close', (code, signal) => {
      ...
      if (turnEnded) {
        console.log(`[${aiTool}] duplicate close ignored (code=${code}, signal=${signal})`);
        return;
      }
      turnEnded = true;
      clearTurnTimers();
      ...
      if (aiTool === 'devin') {
        ...
        // #368 Phase1-4: 「出力ゼロ + exit 0」も失敗扱いに含める（無言終了時に一時ファイル
        // ・export を残し検証可能にするため）。
        const devinTurnFailed =
          ...
          (fullOutput.trim().length === 0 && code === 0);
        const devinPromptTempPath = path.join(os.tmpdir(), `devrelay-prompt-${sessionId}.txt`);
        if (devinTurnFailed) {
          console.log(`[devin] ⚠️ Turn failed, keeping temp files for inspection: ${devinPromptTempPath}${devinExportPath ? ` / ${devinExportPath}` : ''}`);
        } else {
      ...
      // #368 Phase1-5: Devin プランモードでの拒否・無言終了の理由を優先順位付きで1つ選んで返す。
      // 優先順位: 拒否されたコマンド特定（extractBlockedCommands）→ ATIF observation 抜粋
      // （extractRejectionEvidence）→ stderr/ログの拒否検出 → 無言終了検知（endedWithoutAnswer）。
      // どれにも該当しなければ null（呼び出し側でフォールバック文言を出す）。
      // #304 型の同期漏れ再発防止のため、Block A（完全無出力、直後）/ Block B（プランモード無言終了、
      // 完了ブロック内）の2箇所から呼ぶ単一情報源として切り出した。自動リトライは追加しない
      // （#364 の判断を維持、二重課金回避）。
      const emitDevinPlanDiagnosis = (): string | null => {
        const lang = options.language ?? DEFAULT_CHAT_LANGUAGE;
        if (devinAtifBlockedCommands.length > 0) {
          return tChat(lang, 'devin.blockedCommands', { commands: devinAtifBlockedCommands.join('\n   ') });
        }
        if (devinAtifRejectionEvidence !== null && isDevinToolRejectionText(devinAtifRejectionEvidence)) {
          return tChat(lang, 'devin.rejectionEvidence', { evidence: devinAtifRejectionEvidence });
        }
        if (isDevinToolRejectionText(stderrOutput) || devinToolRejectedInLog) {
          const stderrTail = stderrOutput.trim().split('\n').slice(-5).join('\n');
          return tChat(lang, 'devin.planToolRejectedNoRetry', { stderrTail });
        }
        if (devinAtifEndedWithoutAnswer) {
          return tChat(lang, 'devin.planTurnTruncated', { tool: aiTool });
        }
        return null;
      };
      ...
      // #368 Phase1-5: ハードコード日本語1文だけだったものを emitDevinPlanDiagnosis() の判定結果へ差し替え、
      // どれにも該当しなかった場合のみ新規 i18n キー devin.emptyExitZero にフォールバックする。
      if (aiTool === 'devin' && fullOutput.trim().length === 0 && code === 0 && !completionSent) {
        completionSent = true;
        const lang = options.language ?? DEFAULT_CHAT_LANGUAGE;
        const diagnosis = emitDevinPlanDiagnosis();
        const stderrTail = stderrOutput.trim() ? `\n\n[stderr]\n${stderrOutput.trim().split('\n').slice(-5).join('\n')}` : '';
        onOutput(
          diagnosis ?? tChat(lang, 'devin.emptyExitZero', { stderrTail }),
          true,
          result.usageData
        );
        resolve(result);
      ...
      if (!completionSent) {
        ...
        if (fullOutput.length === 0) {
          ...
          // 自動リトライはしない（二重課金回避のため）。
          // #368 Phase1-5: 判定順（拒否コマンド特定 → ATIF証拠 → stderr/ログの拒否検出 →
          // 無言終了）を Block A と共有する emitDevinPlanDiagnosis() に一本化（#304 型の
          // 同期漏れ再発防止）。#368 Phase2a-D: ゲート条件を aiTool==='devin' のみへ再設定
          // （旧プラン専用configの適用有無を前提とした条件は、Devin が常に dangerous 相当で
          // 起動するようになったため恒久的に成立しなくなった）。
          if (aiTool === 'devin') {
            const diagnosis = emitDevinPlanDiagnosis();
            if (diagnosis) {
              onOutput('\n' + diagnosis + '\n', false);
...
export function cancelAiSession(sessionId: string): boolean {
  // #355: Claude SDK 経路（session.process が null のため下の分岐は必ず false を返す）。
  // activeSdkAborts に登録済みの AbortController を実際に abort() することで
  // claude サブプロセスに SIGTERM を送る（`c` コマンドが「キャンセルした」と嘘をつく問題の解消）。
  // 注: macOS Agent は PTY（terminalMode）経路を持たないため、linux 版と異なり PTY 分岐は無い。
