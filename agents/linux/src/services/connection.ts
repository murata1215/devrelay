...
export async function handleConversationClear(payload: { sessionId: string; projectPath: string; agentScopeId?: string }) {
  ...
  console.log(`🗑️ Clearing conversation for session ${sessionId} (scope: ${agentScopeId ?? 'default'})`);
  ...
  try {
    ...
    await clearDevinModel(projectPath, agentScopeId); // このサイクル: モデル情報もセッションIDと一緒にクリア
    await clearDevinAtifStepOffset(projectPath, agentScopeId); // #365: ATIF累計ステップ数オフセットも三つ目の要素として同時にクリアする
    await clearDevinPermissionMode(projectPath, agentScopeId); // #368 Phase2a: パーミッションモード記録も四つ目の要素として同時にクリアする
    await clearCodexSessionId(projectPath, agentScopeId); // #308
    ...
    // 不正な agentScopeId 等で resolveScopeDir() が throw してもプロセスを落とさない
    console.error(`❌ Failed to clear conversation for session ${sessionId} (scope: ${agentScopeId ?? 'default'}):`, (err as Error).message);
  }
}
  ...
  if (sessionInfo) {
    ...
    if (payload.aiTool && payload.aiTool !== sessionInfo.aiTool && currentConfig) {
      ...
      if (resolved !== sessionInfo.aiTool) {
        console.log(`🔄 [exec] aiTool synced from server: ${sessionInfo.aiTool} -> ${resolved}`);
...
async function handleAiPrompt(payload: { sessionId: string; prompt: string; userId: string; files?: FileAttachment[]; missedMessages?: MissedMessage[]; execPrompt?: string; projectPath?: string; aiTool?: AiTool; terminalMode?: boolean; forceNewSession?: boolean; model?: string; isWCommand?: boolean; language?: Language; permissionPolicy?: string; agentScopeId?: string; resumeSessionId?: string; turnId?: string }) {
  ...
  // 履歴注入の判定条件（元コードと同一）:
  //   1. Claude SDK: resumeSessionId がない（SDK の --resume でセッション引き継ぎするため通常は不要）
  //   2. Claude SDK: missed messages がある（SDK 内部履歴にない新規メッセージ）
  //   3. 非 Claude（Devin/Gemini 等）: 常に含める（--resume が効かずプロンプトが唯一のコンテキスト）
  //   Claude terminal mode は除外: Claude CLI 自身が JSONL でセッション管理するため注入不要
  //   （注入すると過去文脈に引きずられ新規質問を無視する症状。2026-05-27 実機 mviewer で確認）
  const composeFullPrompt = (includeHistory: boolean): string => {
    const basePrompt = effectiveModeInstruction + prevExecSummary + '\n\n' + promptWithFiles + workStatePrompt + storageContextPrompt + OUTPUT_DIR_INSTRUCTION;
  ...
  const maybeWarnContext = (usage?: ContextUsage): void => {
    ...
    if (isContextWarning(usage, CONTEXT_WARN_THRESHOLD)) {
      ...
      console.warn(`⚠️ Context warning sent (${usage.percentage}%, hard=${isHard})`);
  ...
  const sendOptions: SendPromptOptions = {
    ...
    // #316: チャット表示言語（'en' の場合のみ AI への英語応答指示が付与される）
  ...
  try {
    ...
    if (aiResult.resumeFailed) {
      ...
      console.log(`📏 Retry prompt size: ${retryPrompt.length} chars (history context: ${historyContextSize} chars)`);
...
async function handleAgentUpdate() {
  ...
  if (process.platform === 'win32') {
    ...
    const scriptLines = [
      ...
      [
        psLog('ensure conpty.node start'),
        ...
        `    "[$(Get-Date -Format 'yyyy-MM-dd HH:mm:ss')] conpty.node missing, fetching: $url" | Out-File -Append "${updateLogFile}"`,
        `    try {`,
        `      Invoke-WebRequest -Uri $url -OutFile $tmp -UseBasicParsing -ErrorAction Stop`,
        `      & tar -xzf $tmp -C $ptyPkg 2>&1 | Out-File -Append "${updateLogFile}"`,
        `      Remove-Item $tmp -Force -ErrorAction SilentlyContinue`,
        `      if (Test-Path "$ptyPkg\\build\\Release\\conpty.node") {`,
        `        "[$(Get-Date -Format 'yyyy-MM-dd HH:mm:ss')] conpty.node extracted successfully" | Out-File -Append "${updateLogFile}"`,
        `      } else {`,
        `        "[$(Get-Date -Format 'yyyy-MM-dd HH:mm:ss')] conpty.node still missing after extract" | Out-File -Append "${updateLogFile}"`,
        `      }`,
        `    } catch {`,
        `      "[$(Get-Date -Format 'yyyy-MM-dd HH:mm:ss')] PTY prebuild download failed: $($_.Exception.Message)" | Out-File -Append "${updateLogFile}"`,
        `    }`,
        `  }`,
        `}`,
    ...
    const distIndexPath = join(agentDir, 'agents', 'linux', 'dist', 'index.js');
 ...
 * Windows（このファイルは Windows CLI Agent としても使われる）では候補が Unix パス想定のため
 * existsSync が false になり無害にスキップされる。
