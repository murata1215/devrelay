...
export const SKILL_INVOCATIONS: readonly SkillInvocation[] = [
  ...
  { name: 'devrelay-read-messages', script: 'scripts/read.sh', readonly: true },
  { name: 'devrelay-list-inventory', script: 'scripts/list.sh', readonly: true },
...
export function getSkillMarkdownBodies(opts?: {
  ...
  return {
    ...
    'devrelay-read-messages': generateReadMessagesSkillMd(opts),
    'devrelay-list-inventory': generateInventorySkillMd(opts),
 ...
 */
function generateReadMessagesSkillMd(opts?: SkillMdOptions): string {
  return `---
...
${windowsInvocationNote('devrelay-read-messages', opts)}
 ...
 */
function generateInventorySkillMd(opts?: SkillMdOptions): string {
  return `---
...
## DevRelay インベントリ一覧
${windowsInvocationNote('devrelay-list-inventory', opts)}
...
export async function ensureSkillFiles(config: AgentConfig): Promise<void> {
  ...
  try {
    ...
    await fs.writeFile(readMsgSkillMdPath, generateReadMessagesSkillMd(skillMdOpts), 'utf-8');
    ...
    await fs.writeFile(inventorySkillMdPath, generateInventorySkillMd(skillMdOpts), 'utf-8');
