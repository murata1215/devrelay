  ...
  return new Promise<AiRunResult>((resolve) => {
    ...
    proc.on('close', (code, signal) => {
      ...
      if (aiTool === 'devin') {
        ...
        if (devinTurnFailed) {
          console.log(`[devin] ⚠️ Turn failed, keeping temp files for inspection: ${devinPromptTempPath} / ${devinPlanConfigTempPath}${devinExportPath ? ` / ${devinExportPath}` : ''}`);
        } else {
