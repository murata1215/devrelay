...
// サイクル SDK-1: getClaudeExecutableFallback() / logClaudeExecutableStatus() は
...
export async function sendPromptToAi(
  ...
  if (aiTool === 'gemini') {
    ...
    // #344: 理由を enum 化。#368 Phase2a-D: Devin は常に exec 相当（dangerous）で起動するようになったため
    // プラン専用の劣化理由は削除、exec モードの --permission-mode 非対応のみを表す。
    ...
    // #368 Phase2a-D: Devin は常に exec 相当（dangerous）で起動する（旧来の plan 系分岐①〜④は
    // usePlanMode が Devin では常に false になり到達しなくなったため削除済み、Cycle C で導出した
    // devinTurnPermissionMode をそのまま使う）。プラン中の読み取り専用強制は git-guard（自動復元、
    // git-guard.ts）+ preamble（devin-plan-prompt.ts のプロンプト指示）に委譲する。
  ...
  return new Promise<AiRunResult>((resolve) => {
    ...
    proc.on('close', (code, signal) => {
      ...
      // #368 Phase1-4: ATIF export ファイルの削除はここでは行わず、下の devinTurnFailed 判定後
      // （devin ブロック内）へ移動した。失敗ターンではファイルを温存し事後調査できるようにする。
      // #277: 実行時間上限タイマー停止
      ...
      if (aiTool === 'devin') {
        ...
        // #368 Phase1-4: 「出力ゼロ + exit 0」（無言終了）も失敗扱いに加える。この場合の ATIF export
        // ファイルは事後調査の最有力手がかりのため、他の一時ファイルと同じ保全ルールに乗せる。
