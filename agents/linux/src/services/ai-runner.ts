...
export async function sendPromptToAi(
  ...
  if (aiTool === 'gemini') {
    ...
    if (options.usePlanMode && !options.devinAutoPermFallback && (devinHasConfig || devinHasAgentConfig)) {
      // plan モード: --config（後継、#347）または --agent-config（旧、後方互換）で Read のみ許可、Write/Exec を明示的に deny
      // --permission-mode auto は「安全と判断したツールを自動承認」するだけで
      // 厳密な読み取り専用ではないため、config で強制する
      ...
      // #329/#347: --config/--agent-config 非対応の CLI → --permission-mode auto に劣化（読み取り専用強制はプロンプト指示のみ）
