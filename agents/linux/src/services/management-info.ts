...
function generateLinuxInfo(): ManagementInfo {
  ...
  const commands: ManagementCommand[] = [
    ...
    { type: 'stop', label: '停止', command: '{ pgrep -u $(whoami) -f "\\.devrelay.*index\\.js"; pgrep -u $(whoami) -fx "node index\\.js"; } 2>/dev/null | sort -u | grep -v "^$$\\$" | xargs -r kill' },
    {
      type: 'restart',
      label: '再起動',
      ...
      command: `NODE_BIN="${nodePath}"; [ ! -x "$NODE_BIN" ] && NODE_BIN=node; { pgrep -u $(whoami) -f "\\.devrelay.*index\\.js"; pgrep -u $(whoami) -fx "node index\\.js"; } 2>/dev/null | sort -u | grep -v "^$$\\$" | xargs -r kill 2>/dev/null || true; sleep 1; cd ${path.dirname(agentIndex)} && nohup "$NODE_BIN" ${agentIndex} < /dev/null >> ${logFile} 2>&1 &`,
    },
