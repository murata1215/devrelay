...
// Agreement バージョン
export const AGREEMENT_VERSION = 'v6';
...
// rules/devrelay.md に書き込む Agreement 全文テンプレート（デフォルト）
// WebUI Settings から編集可能。カスタムテンプレートが DB に保存されていればそちらを優先する。
export const DEFAULT_RULES_TEMPLATE = `<!-- DevRelay Agreement ${AGREEMENT_VERSION} -->
 ...
 * @param customTemplate ユーザーがカスタマイズしたテンプレート（省略時はデフォルト使用）
 * @returns Claude Code に渡す Agreement 適用プロンプト全文
 */
export function buildAgreementApplyPrompt(customTemplate?: string): string {
  const rulesTemplate = customTemplate ?? DEFAULT_RULES_TEMPLATE;
