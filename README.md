...
- **Terminal Interface Mode**: Per-project "端末" toggle launches `claude` via PTY (`@homebridge/node-pty-prebuilt-multiarch` + `@xterm/headless`) instead of Agent SDK — on-demand start/stop with no memory residency. Works on Linux + Windows (CLI install). Resumes the SDK's session via `claude --resume <id>` so SDK and terminal mode share `~/.claude/projects/<hash>/sessions/<id>.jsonl` and continuity flows in both directions. 250ms polling detects trust folder prompts and input prompts even during PTY silence; numbered choice prompts (tool approvals, AskUserQuestion, trust dialog) are bridged to the existing WS approval card flow — WebUI/Discord/Telegram unchanged. Completion fires only when the screen is idle 1.5s, prompt is ready, AND at least one new `●` bullet has appeared — the bullet gate prevents premature completion during Claude's thinking-phase PTY silence (round-trip latency between submit and first token). Windows installer auto-downloads ABI-matching `conpty.node` from GitHub Releases when `prebuild-install` skips it
