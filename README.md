...
- **Chat Language (#316)**: The same `UserSettings.language` preference now also applies to chat command replies (`a`/`h`/`m`/`p`/`s`/`c`/`x`/`w` etc., resolved fresh on every `getUserContext()` call) and to the AI itself — when set to English, a short "respond in English" directive is appended to the prompt sent to the Agent so Claude/Codex/Gemini/Devin answer in English too. Unlike the WebUI (`'en'` default), the chat default is `'ja'` so existing users aren't suddenly switched to English. Note: Agent progress messages (e.g. "⏳ running command") and some less-common commands (`link`/`agreement`/`session`/`build`/`log`/`summary`/`kill`/`u`/`quit`/model settings/`testflight`/`ask`/`teamexec`) are not yet localized.
