...
- **raw-completion API (Phase 1, Claude only)**: `POST /api/agent/raw-completion` — a dedicated, machine-token-authenticated endpoint for external callers (e.g. an LLM board-game engine) that want a project's Agent to act as a bare completion API for one turn, with a fully-replaced `system` prompt and zero DevRelay preamble/Agreement/plan-mode instructions mixed in. Built as a structural bypass of `ask`/`teamexec`/MCP exec (separate `server:raw:prompt`/`agent:raw:result` WebSocket channel, separate `raw_`-prefixed session ID, in-memory per-seat concurrency=1 + rate limiting) so the existing chat/MCP code paths are untouched. Four-layer tool lockdown (`tools:[]` + `disallowedTools` + unconditional `canUseTool` deny + `permissionMode:'plan'`, plus `mcpServers:{}`/`strictMcpConfig:true` and `settingSources:[]`) prevents the caller-supplied prompt from reading project files. Linux/macOS Agents only — the Windows Electron agent doesn't import the Claude Agent SDK so it can't replace the system prompt and doesn't advertise the capability. **Response contract (Phase 1.1, breaking change from Phase 1)**: `{text, output, model, usage:{input,output,cacheRead,cacheWrite}, latencyMs, agentDurationMs, stopReason, sessionId, error?, deniedTools}` — every key is always present. `output` is a deprecated alias kept equal to `text` for older callers; new integrations should read `text`. Callers **must check `stopReason !== 'success'`** before trusting `text` as a complete answer (`'max_turns'`/`'aborted'` return partial output rather than hiding the truncation, `'error'`/`'timeout'` return an empty `text` with `error` set). `deniedTools` is a deduplicated list of tool names the Agent's `canUseTool` actually denied — always `[]` in the normal case; a non-empty list means the four-layer lockdown was reached, which is logged server-side (`console.warn` + a `role:'system'` Message row) since Agent's `agent.log` isn't readable by the server's OS user. Requires the target Agent to have run `u` (self-update) at or after Phase 1.1 — older Agents omit `text` entirely, which the server detects and surfaces as `error: "...outdated agent..."` instead of a silent empty body
