...
- **Terminal Interface Mode**: Per-project "端末" toggle launches `claude` via PTY (`@homebridge/node-pty-prebuilt-multiarch` + `@xterm/headless` with 10000-line scrollback) instead of Agent SDK — on-demand start/stop with no memory residency. Works on Linux + Windows (CLI install). Resumes the SDK's session via `claude --resume <id>` so SDK and terminal mode share `~/.claude/projects/<hash>/sessions/<id>.jsonl` and continuity flows in both directions. **Any numbered-choice prompt** (trust folder, resume-from-summary, tool approvals, AskUserQuestion, future CLI system prompts) is automatically bridged to the existing WS approval card flow — user decides via WebUI/Discord/Telegram, agent never auto-confirms. Live progress visibility during processing: new `●` bullets stream to WebUI as they appear, and 30s heartbeats surface Claude's thinking indicator (`⏳ [60s] Doing (109 tokens)`). Completion fires when screen idle 1.5s + prompt ready + at least one new bullet (tracked by `Map<text, count>` per-text delta so it survives both scrollback trim and re-asks where the response matches existing JSONL history). Completion check polls a fresh `extractFinalOutput(term)` each 500ms tick to avoid xterm tracker freeze. Three safety nets: 1.5s idle gate, 10min first-bullet timeout, 10min onData-based idle timeout. Windows installer auto-downloads ABI-matching `conpty.node` from GitHub Releases when `prebuild-install` skips it
