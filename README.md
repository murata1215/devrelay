...
- **Devin Plan Mode Read-Only Enforcement (redesigned in #363)**: Devin plan mode enforces read-only via `--agent-config`/`--config`. A pure module `devin-plan-config.ts` (byte-for-byte identical across all three OS agents, zero external imports) builds the config from an explicit **Exec allow-list** (read-only bash commands from `PLAN_READONLY_BASH_COMMANDS`, git read-only subcommand prefixes — `git log`/`git status`/`git diff`/`git show`/`git branch` — plus DevRelay's own skill-directory bash-invocation prefixes, e.g. `Exec(bash "<skills-dir>/...)`, covering quoted/unquoted and POSIX/Windows path separators, so bash-based investigation skills like `devrelay-list-inventory` aren't blocked) and an **Exec deny-list** (write commands from `PLAN_WRITE_BASH_COMMANDS` plus `sudo`), layered on top of the base `allow: ["Read(**)"]` / `deny: ["Write(**)"]` (deny always wins over allow, so `Write(**)` stays blocked; `version`/`shell.setup_complete` are still included to suppress the first-run banner, #347). **`Exec(**)` itself was never a valid rule**: `Exec()` is prefix-matching, not glob, and `Exec(**)` is explicitly documented as invalid — so the original `deny: ["Write(**)","Exec(**)"]` (#260) never blocked anything, and #362's "remove `Exec(**)` from deny" fix was correct but incomplete, since it left DevRelay's own skills with no matching rule at all (falling through to a silent approval-wait — Devin's non-interactive mode never prints rejection text, exit 0). A second pure function, `resolveDevinPlanPermissionMode()`, layers `--permission-mode` on top: **`auto` alone only auto-approves read-only tools, never shell/`Exec` calls** (verbatim from `devin --help`) — this was the actual root cause of plan-mode skill calls silently hanging — so DevRelay now prefers `--permission-mode smart` (probed via `--help`; additionally auto-runs actions a fast model judges safe) and falls back to `auto` only when `smart` isn't offered. Kill switches: `DEVRELAY_DEVIN_PLAN_EXEC_DENY=1` restores the minimal `allow: ["Read(**)"]` / `deny: ["Write(**)"]`-only config with `--permission-mode auto` (no Exec allow/deny at all, today's pre-#363 behavior); `DEVRELAY_DEVIN_PLAN_PERMISSION_MODE=auto|smart` overrides the mode explicitly. Exec mode is unchanged (`--permission-mode dangerous`). The temp config is auto-deleted on process exit alongside the existing prompt temp file (#260)
