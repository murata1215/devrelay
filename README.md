...
- **Per-Agent Remote Restart**: Restart individual agents from WebUI (table row icon + Settings modal button) via WebSocket `server:agent:restart` → Linux/macOS rely on systemd/launchd auto-restart, Windows self-relaunches via `wscript.exe start-agent.vbs` (detached spawn) before `process.exit(0)`. Server-side `connectedAgents` registration is done immediately after token verification (before async DB operations) to prevent race conditions where the old agent's disconnect event fires mid-`handleAgentConnect` and clobbers the new connection
