...
- **Chat Language (#316, #318, #319)**: The same `UserSettings.language` preference now also applies to chat command replies (`a`/`h`/`m`/`p`/`s`/`c`/`x`/`w`/`link`/`agreement`/`log`/`summary`/`kill`/`u`/`quit`/disconnect/`session`/`build` etc., resolved fresh on every `getUserContext()` call, or via `resolveSessionLanguage(sessionId)` for handlers that only have a sessionId), the live progress box (`🤖 Processing... / ⏱️ Ns elapsed / 🔧 Using Bash...` / `✅ Done`), the DevRelay Agreement status line (`✅ DevRelay Agreement up to date`), Codex/Devin step summaries in the Agent's live output, and the AI itself — when set to English, a short "respond in English" directive is appended to the prompt sent to the Agent so Claude/Codex/Gemini/Devin answer in English too. Unlike the WebUI (`'en'` default), the chat default is `'ja'` so existing users aren't suddenly switched to English. Note: `testflight` and cross-project `ask`/`teamexec` are not yet localized (testflight also writes Japanese scaffolding templates to disk as `CLAUDE.md`/rules files for the AI, which is a separate concern from chat replies).
