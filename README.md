...
- **Token Bloat Warning**: When a project's recent conversations stay token-heavy — 3 consecutive turns each ≥3000k tokens, or a 10-turn average ≥2000k (mostly `cache_read`, the context re-read every turn) — the completion message prepends a `⚠️` nudge to run `w` (record & commit) then `x` (clear history), which actually resets the Claude session and drops cost/latency. Per-project 60-min cooldown so it nudges at most once an hour; thresholds tunable via `DEVRELAY_TOKEN_WARN_*` env (or `DEVRELAY_TOKEN_WARN_DISABLED=1`). Complements the per-session context-% warning (#291)
