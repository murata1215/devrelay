...
- **Devin Plan Mode Read-Only Enforcement**: Devin plan mode now enforces true read-only at the tool level via `--agent-config`/`--config`. Plan mode writes a temp JSON (`{"permissions":{"allow":["Read(**)"],"deny":["Write(**)"]}}`, plus `version`/`shell.setup_complete` to suppress the first-run banner, #347) and passes it via config, denying all file writes (deny always wins over allow, so `Write(**)` stays blocked). Where `--permission-mode` is also available, it's layered on top as `--permission-mode auto` for defense-in-depth (Devin's own "auto-approve tools it deems safe" judgment). **`Exec(**)` was removed from the deny list in #362**: the original `deny: ["Write(**)","Exec(**)"]` (#260) structurally blocked DevRelay's own bash-based investigation skills (e.g. `devrelay-list-inventory`) during plan mode, and since Devin's non-interactive deny is completely silent (exit 0, no rejection text, #347 Phase0), a denied skill call looked exactly like the AI answering with a preamble and then going quiet mid-turn. A killswitch (`DEVRELAY_DEVIN_PLAN_EXEC_DENY=1`) restores the old all-Exec-denied behavior without `--permission-mode`. Exec mode is unchanged (`--permission-mode dangerous`). The temp config is auto-deleted on process exit alongside the existing prompt temp file (#260)
