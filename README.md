...
- **Chat Language (#316, #318)**: The same `UserSettings.language` preference now also applies to chat command replies (`a`/`h`/`m`/`p`/`s`/`c`/`x`/`w`/`link`/`agreement`/`log`/`summary`/`kill`/`u`/`quit`/disconnect etc., resolved fresh on every `getUserContext()` call), the live progress box (`🤖 Processing... / ⏱️ Ns elapsed / 🔧 Using Bash...`), and the AI itself — when set to English, a short "respond in English" directive is appended to the prompt sent to the Agent so Claude/Codex/Gemini/Devin answer in English too. Unlike the WebUI (`'en'` default), the chat default is `'ja'` so existing users aren't suddenly switched to English. Note: a handful of less-common commands (`session`/`build`/model settings/`testflight`/`ask`/`teamexec`) are not yet localized.
