...
- **Token Bloat Warning**: When a project's recent conversations stay token-heavy — 3 consecutive turns each ≥3000k tokens, or a 10-turn average ≥2000k (mostly `cache_read`, the context re-read every turn) — the completion message prepends a `⚠️` nudge to run `w` (record & commit) then `x` (clear history), which actually resets the Claude session and drops cost/latency. **#321 added a third "slow" axis**: 3 consecutive turns each ≥25k output tokens AND ≥5min duration triggers a `🐢` nudge (catches turns that got slow/verbose without necessarily accumulating a huge `cache_read` total). Per-project 60-min cooldown so it nudges at most once an hour; thresholds tunable via `DEVRELAY_TOKEN_WARN_*` env (or `DEVRELAY_TOKEN_WARN_DISABLED=1`). Message text is localized via `tChat()`/`resolveSessionLanguage()`. Complements the per-session context-% warning (#291)
