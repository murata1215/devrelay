...
- **Devin Plan Mode Read-Only Enforcement (redesigned in #363)**: Devin plan mode enforces read-only via `--agent-config`/`--config`. A pure module `devin-plan-config.ts` (byte-for-byte identical across all three OS agents, zero external imports) builds the config from an explicit **Exec allow-list** (read-only bash commands from `PLAN_READONLY_BASH_COMMANDS`, git read-only subcommand prefixes — `git log`/`git status`/`git diff`/`git show`/`git branch` — plus DevRelay's own skill-directory bash-invocation prefixes, e.g. `Exec(bash "<skills-dir>/...)`, covering quoted/unquoted and POSIX/Windows path separators, so bash-based investigation skills like `devrelay-list-inventory` aren't blocked) and an **Exec deny-list** (write commands from `PLAN_WRITE_BASH_COMMANDS` plus `sudo`), layered on top of the base `allow: ["Read(**)"]` / `deny: ["Write(**)"]` (deny always wins over allow, so `Write(**)` stays blocked; `version`/`shell.setup_complete` are still included in the generated config, but **`shell.setup_complete` does NOT actually suppress Devin's first-run banner** — corrected in #364 Phase0.5 after real-hardware testing showed the `Welcome to Devin CLI!` banner still prints even with `shell.setup_complete:true`; the only effective defense is output-side filtering via `isDevinBannerLine()` in `devin-diagnostics.ts`). **`Exec(**)` itself was never a valid rule**: `Exec()` is prefix-matching, not glob, and `Exec(**)` is explicitly documented as invalid — so the original `deny: ["Write(**)","Exec(**)"]` (#260) never blocked anything, and #362's "remove `Exec(**)` from deny" fix was correct but incomplete, since it left DevRelay's own skills with no matching rule at all, falling through to an approval-wait. **Corrected in #364 Phase0.5**: contrary to earlier documentation, Devin's non-interactive deny is *not* silent — it prints a warning (`warning: rejected a tool call that requires confirmation. proceeding without it.`, or on older builds `A tool was rejected by the user` / `rejecting tool ... that requires confirmation`) before exiting 0; this text is what `isDevinToolRejectionText()` detects. A second pure function, `resolveDevinPlanPermissionMode()`, layers `--permission-mode` on top: **`auto` alone only auto-approves read-only tools, never shell/`Exec` calls** (verbatim from `devin --help`) — this was the actual root cause of plan-mode skill calls silently hanging. Devin's `--permission-mode` actually has **four** modes: `auto` (default), `accept-edits`, `smart`, `dangerous` (`accept-edits` was undocumented here until #364 Phase0.6 real-hardware testing found it). #363 initially made DevRelay prefer `--permission-mode smart` (probed via `--help`) whenever offered, falling back to `auto` otherwise — but **#364 Phase1 reverted this**: real-hardware testing (Phase0.6) showed `smart` can be unavailable for server-side reasons and silently falls back to Devin's own `normal` mode (`Warning: Smart permission mode is not available. Falling back to normal.`, observed 5 times out of 5 attempts), and the `--help`-string probe cannot detect this unavailability. DevRelay's default is therefore now unconditionally `auto` regardless of `smart` support. Separately, ATIF's `agent.extra.permission_mode` field reports `auto` mode using the human-readable label `"Normal"` — this is purely a display artifact of ATIF and is not evidence that `smart` failed to apply. Kill switches: `DEVRELAY_DEVIN_PLAN_EXEC_DENY=1` restores the minimal `allow: ["Read(**)"]` / `deny: ["Write(**)"]`-only config with `--permission-mode auto` (no Exec allow/deny at all, today's pre-#363 behavior); `DEVRELAY_DEVIN_PLAN_PERMISSION_MODE=auto|smart` overrides the mode explicitly and always takes precedence. Exec mode is unchanged (`--permission-mode dangerous`). The temp config is auto-deleted on process exit alongside the existing prompt temp file (#260), except that on abnormal termination DevRelay now keeps both files for post-mortem inspection instead of always deleting them (#364 Phase1). **#364 Phase1 also narrows and clarifies the failure-reporting path**: the `devinPlanToolRejected` auto-retry gate is restored to firing only on a genuine silent failure (`fullOutput.trim().length === 0`, not merely "rejection text was seen somewhere"), so a preamble sentence followed by a rejection no longer triggers a wasted double-billed retry; untimestamped stderr-line detection for both rejection text and a new `Smart permission mode is not available. Falling back to normal.` line is centralized in `classifyDevinStderrLine()`/`devin-diagnostics.ts`. When the last ATIF step is a rejected tool call, DevRelay now extracts a 300-character excerpt straight from `observation.results[].content` for **that failed turn only** (`extractRejectionEvidence()`, `devin-atif.ts`) and shows it verbatim as `devin.rejectionEvidence` instead of a generic message — the one narrow, intentional exception to `devin-atif.ts`'s pre-#361 "never surface `observation`" rule; `summarizeAtifEntry()` itself is unchanged. The final per-turn message is now a 3-way branch (ATIF rejection evidence → stderr/log rejection → `endedWithoutAnswer` truncation), gated only on whether the plan config was applied, so a reason is always shown instead of occasionally staying silent. **#364 1-B (真因A, root cause A) — broken skill Exec rules fixed**: the Exec allow-list entries DevRelay generated for its own skill scripts were syntactically broken (`buildSkillExecPrefixes()` produced strings like `Exec(bash "<skills-dir>/...)` with an unclosed quote mid-token) and could never match Devin's actual token-prefix-matching rule against the tilde-form command DevRelay's skills invoke — confirmed via real-hardware testing (Phase0.6) that `Exec()` matches whole argv tokens, not a glob or raw substring. `buildSkillExecPrefixes()` has been deleted; `skill-manager.ts` now owns a single source of truth, `SKILL_INVOCATIONS`, driving both the generated `SKILL.md` body text (`getSkillMarkdownBodies()`) and the exact command strings Devin must be allowed to run (`buildSkillInvocationCommands()`), so the two can never drift apart again (enforced by a regex-scan test, `skill-invocations.test.mjs`). `devin-plan-config.ts`'s `DevinPlanConfigOptions.skillsDir: string` was replaced with `skillExecCommands: readonly string[]` — callers pre-build the exact commands, keeping the config module independent of `skill-manager.ts`. **#364 1-C (真因B, root cause B) — Windows bash path-namespace fix**: on Windows, `bash` resolves to **WSL** bash when invoked via Devin's non-interactive exec but to **Git bash** when invoked via Claude Code's own Bash tool, and these require mutually exclusive path forms (WSL needs `/mnt/c/...`, Git bash needs `/c/...`) — confirmed by real 4-quadrant hardware testing on a live Windows machine. A new pure module `windows-skill-path.ts` (`toWslPath()`/`toGitBashPath()`, byte-for-byte identical across the linux/macos agents, zero external imports) converts Windows paths to both forms; `buildSkillInvocationCommands({skillsDir, platform:'win32'})` now also emits WSL-form and Git-bash-form invocation commands (quoted and unquoted), and generated `SKILL.md` bodies gain a Windows-only usage note (`windowsInvocationNote()`) instructing the AI to try the WSL form first, fall back to the Git-bash form only if that fails, and never merge the two into a single compound command with `;`/`&&` — Devin's permission layer rejects compound commands outright even when every individual command would otherwise be allowed. Non-Windows `SKILL.md` bodies remain byte-identical to before this fix
...
- **Devin Plan-Mode Silent-Termination Diagnosis Overhaul, Phase 1 of 4 (#368)**: Root cause found for "exec が実行されず無言終了する" reports: Devin's non-interactive permission check decomposes compound shell commands (`a && b`, `a ; b`, pipes) into **individual bare command names**, so the `Exec()` prefix-matching allow-list approach used since #260 (see #363 above) is structurally unreliable — no amount of allow-list tuning can keep up with every possible command combination. Separately, if *any one* tool call in a batch of parallel tool calls is rejected, Devin cancels the entire turn via an "Interrupting stop token," exiting 0 with zero output text — this is the actual "silent termination." A 4-phase fix was scoped (superseding the abandoned `keen-purring-crystal.md` plan); **only Phase 1 (visibility, no architecture change) shipped this cycle**: `devin-atif.ts` gained `extractBlockedCommands()`, a reverse-scan of ATIF entries that surfaces the *actual rejected command string* to chat instead of a generic message (falling back to `devin.emptyExitZero` with a stderr excerpt when no evidence is available), and two previously-duplicated diagnosis code paths in `ai-runner.ts` were consolidated into a single `emitDevinPlanDiagnosis()` to prevent the drift bugs this project has repeatedly hit (#304-class sync misses). ATIF export files are now preserved on abnormal turn termination instead of being deleted immediately, for post-mortem inspection. **Phase 2 (removing `devin-plan-config.ts`'s `Exec()` allow/deny generation entirely), Phase 3 (`p,`/`i`/`revert` commands + git guards), and Phase 4 (Agreement v6→v7) remain unimplemented** — Phase 2 was attempted mid-cycle but left the three OS agents inconsistent and non-building after a session interruption, so it was reverted back to its pre-cycle state rather than shipping broken code, and is deferred to a future cycle in full.
